...
// 本程序使用直接插入排序算法。
// 先创建一个空数组，再将原数组中的数据依次按优先级顺序从大到小插入空数组中，直到原数组被遍历一遍。
// 结束之后空数组中就被填入了有序的数据。
#include <stdio.h>
// 定义三种排序优先级不同的函数
...
// 以表格的形式输出排序后的数据
void print(type agency[], int limit)
{
	int i = 0;
	printf("%-25s%-15sstudent ID  tlscore objscore sbjscore\n", "Major", "Class");
	for (i = 0; i < limit; i++)
	{
		printf("%-15s	", agency[i].major);
		printf("%10s	", agency[i].Class);
		printf("%10d	", agency[i].stdID);
		printf("%-8d", agency[i].tlscore);
		printf("%-8d", agency[i].objscore);
		printf("%-8d\n", agency[i].sbjscore);
	}
}
...
void sort_tlscore(int limit)
{
	...
	for (i = 1; i < limit; i++)
	{
		...
		{
			// 寻找数据应该插入的空位
			if (agency[j].tlscore > data[i].tlscore || (agency[j].tlscore == data[i].tlscore && agency[j].sbjscore > data[i].sbjscore) || (agency[j].tlscore == data[i].tlscore && agency[j].sbjscore == data[i].sbjscore && agency[j].objscore >= data[i].objscore))
			{
				if (agency[j + 1].tlscore < data[i].tlscore || (agency[j + 1].tlscore == data[i].tlscore && agency[j + 1].sbjscore < data[i].sbjscore) || (agency[j + 1].tlscore == data[i].tlscore && agency[j + 1].sbjscore == data[i].sbjscore && agency[j + 1].objscore < data[i].objscore))
				{
					// 如果排在所有已填入数据的最后，直接填入
					if (agency[j + 1].tlscore == 0)
					{
						agency[j + 1] = data[i];
						break;
					}
					// 否则将插入位置之后已填入的数据通过依次交换的方式统一前移一位，
					// 再将要填入的数据放进空位里
					else
					{
						for (k = i - 1; k >= j + 1; k--)
...
void sort_objscore(int limit)
{
	...
	for (i = 1; i < limit; i++)
	{
		...
		{
			// 寻找数据应该插入的空位
			if (agency[j].objscore > data[i].objscore || (agency[j].objscore == data[i].objscore && agency[j].tlscore > data[i].tlscore) || (agency[j].objscore == data[i].objscore && agency[j].tlscore == data[i].tlscore && agency[j].sbjscore >= data[i].sbjscore))
			{
				if (agency[j + 1].objscore < data[i].objscore || (agency[j + 1].objscore == data[i].objscore && agency[j + 1].tlscore < data[i].tlscore) || (agency[j + 1].objscore == data[i].objscore && agency[j + 1].tlscore == data[i].tlscore && agency[j + 1].sbjscore < data[i].sbjscore))
				{
					// 如果排在所有已填入数据的最后，直接填入
					if (agency[j + 1].objscore == 0)
					{
						agency[j + 1] = data[i];
						break;
					}
...
void sort_sbjscore(int limit)
{
	...
	for (i = 1; i < limit; i++)
	{
		...
		{
			// 寻找数据应该插入的空位
			if (agency[j].sbjscore > data[i].sbjscore || (agency[j].sbjscore == data[i].sbjscore && agency[j].tlscore > data[i].tlscore) || (agency[j].sbjscore == data[i].sbjscore && agency[j].tlscore == data[i].tlscore && agency[j].objscore >= data[i].objscore))
			{
				if (agency[j + 1].sbjscore < data[i].sbjscore || (agency[j + 1].sbjscore == data[i].sbjscore && agency[j + 1].tlscore < data[i].tlscore) || (agency[j + 1].sbjscore == data[i].sbjscore && agency[j + 1].tlscore == data[i].tlscore && agency[j + 1].objscore < data[i].objscore))
				{
					// 如果排在所有已填入数据的最后，直接填入
					if (agency[j + 1].tlscore == 0)
					{
